...
def launch_setup(context, *args, **kwargs):
    ...
    # Always preload additional controllers in inactive state
    scaled_joint_trajectory_controller_spawner_stopped = Node(
        package="controller_manager",
        executable="spawner",
        arguments=[
    ...
    # Spawn controllers only after robot has been inserted in Gazebo and the
    # ros2_control plugin had a chance to bring up /controller_manager.
    delay_controller_spawners_after_robot_spawn = RegisterEventHandler(
        event_handler=OnProcessExit(
            target_action=gz_spawn_entity,
            on_exit=[
                joint_state_broadcaster_spawner,
                scaled_joint_trajectory_controller_spawner_stopped,
    ...
    nodes_to_start = [
        ...
        gz_sim_bridge,
        delay_controller_spawners_after_robot_spawn,
    ]
...
def generate_launch_description():
    ...
    declared_arguments.append(
        DeclareLaunchArgument(
            ...
            default_value=PathJoinSubstitution(
                [FindPackageShare("fctr_ur_package"), "config", "ur_controllers.yaml"]
            ),
    ...
    declared_arguments.append(
        DeclareLaunchArgument(
            "description_file",
            default_value=PathJoinSubstitution(
                [FindPackageShare("fctr_ur_package"), "urdf", "ur_gz.urdf.xacro"]
            ),
    ...
    declared_arguments.append(
        DeclareLaunchArgument(
            "rviz_config_file",
            default_value=PathJoinSubstitution(
                [FindPackageShare("fctr_ur_package"), "rviz", "view_robot.rviz"]
            ),
